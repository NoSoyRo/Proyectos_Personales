...
class three_d(ThreeDScene):
    def construct(self):
        ejes = ThreeDAxes()
        circle = Circle()
        self.set_camera_orientation(phi=80 * DEGREES,theta=45*DEGREES)
        self.play(ShowCreation(circle),ShowCreation(ejes))
        self.wait()
        ...
        curva1 = ParametricFunction(lambda t: np.array([np.cos(t), np.sin(t),t]),color=RED,t_min=-TAU,t_max=TAU)
        self.play(ShowCreation(curva1))
        curva2 = ParametricFunction(lambda t: np.array([np.cos(t), np.sin(t),2*t]),color=RED,t_min=-TAU,t_max=TAU)
        curva2.set_shade_in_3d(True)
        self.play(Transform(curva1, curva2))
        #superficie1 = ParametricSurface(lambda u,v: np.array([(1+np.cos(u))*np.cos(v),
        #(1+np.cos(u))*np.sin(v),
        #np.sin(v)
        #]),v_min=0,v_max=2*PI,u_min=0,u_max=2*PI)
        #self.play(Write(superficie1))
        #self.remove(superficie1)
        pto = Arrow((0,0,0),(1,1,1), buff = 0)
        #self.add_fixed_in_frame_mobjects(pto)
        self.play(Write(pto))
        self.wait()