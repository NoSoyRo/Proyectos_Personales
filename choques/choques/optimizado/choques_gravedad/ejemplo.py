import choques_gravitatorios as ch
import numpy as np
...
# L = [ch.particula(1, np.array([1,1]), np.array([-1,0]), 0.5, 0),
# ch.particula(1, np.array([0,0]), np.array([0,1]), 0.5, 1)]

# L = [ch.particula(1, np.array([1,1]), np.array([-1,0]), 0.5, 0)]

exp = ch.experimento(L, 560, 5)
exp.realiza_el_experimento(1000, 0.01)
