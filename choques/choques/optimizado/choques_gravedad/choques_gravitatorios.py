...
class experimento():
    ...
    def realiza_el_experimento(self, num_de_pasos, dt):
        ...
        inicio = time.time()
        din2.write(f'   El proceso comienza a las {inicio}')
        din2.close()
        for i in range(num_de_pasos):
            ...
            for k in self.lp:
                k.v = k.v + self.dt * self.a
                k.p = k.p + self.dt*k.v - ((self.dt**2)/2) * self.a  ### aqui solo cambio las posiciones con las posiciones debidas a un potencial!!!! ueuwuwuwuwuwuuwuwuwuw
        din2 = open('info_util.txt', 'a')
        din2.write(f'   El proceso terminó a las {time.time()}')
        din2.write(f'   El proceso tardó {time.time() - inicio}')
        din2.close()                
