...
lista = [np.array([1,1+0.1]), np.array([1,22]), np.array([1,1])]
num_exps = 100
...
for k in range(num_exps):
    ...
    n_parts          = []
    for i in range(5000, 50001, 5000):#300001
...
for i,j in zip(lista_de_lista_de_tiempos_de_arbol, lista_de_lista_de_tiempos_de_fzabr):
    plt.plot(n_parts, i,label = 'arbol')
    plt.plot(n_parts, j,  label = 'fuerza bruta')
    plt.legend()
plt.show()
